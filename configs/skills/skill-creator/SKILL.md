...
3. Initialize the skill (create folder + SKILL.md template)
4. Edit the skill (implement resources and write SKILL.md)
5. Validate the skill in-repo
...
In this repository, initialize skills manually by creating the target directory and writing a valid `SKILL.md` file with frontmatter.
...
Builtin skills should live under `configs/skills/<skill-name>/SKILL.md`.
...
Add concise execution instructions here.
EOF
```
...
# then create <workspace>/skills/my-skill/SKILL.md with the same structure
```

This manual initialization flow:

- Creates the skill directory at the specified path
- Creates a valid `SKILL.md` with required `name` and `description`
- Lets you add optional resource directories (`scripts/`, `references/`, `assets/`) only when needed

After initialization, customize `SKILL.md` and add resources as needed.
...
Once development of the skill is complete, validate it directly in this repository.

```bash
go test ./pkg/skill -run TestLoadRealConfigSkills -count=1
```
...
2. **Runtime loading**
   - New skill appears in the loaded skill set
   - Skill metadata (for example `homepage`, `metadata.nanobot.requires`) is parsed correctly
3. **Compatibility**
   - Existing skills continue to load correctly (no regressions)

If tests fail, fix the frontmatter or file structure and rerun the same test commands until green.
